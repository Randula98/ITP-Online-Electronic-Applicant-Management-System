...
supplier -
	rankingRoutes.route("/supplier-ranking").get(function (req, res) {
...
supplier -
	rankingRoutes.route("/supplier-ranking/:id").get(function (req, res) {
		let db_connect = dbo.getDb("synthetic");
...
supplier -
	rankingRoutes.route("/supplier-ranking/add").post(function (req, response) {
		let db_connect = dbo.getDb("synthetic");
		let myobj = {
...
supplier -
	rankingRoutes.route("/supplier-ranking/update/:id").post(function (req, response) {
		let db_connect = dbo.getDb("synthetic");
		let myquery = { _id: ObjectId(req.params.id) };
		let newvalues = {
			$set: {
...
supplier -
	rankingRoutes.route("/supplier-ranking/delete/:id").delete((req, response) => {
		let db_connect = dbo.getDb("synthetic");
		let myquery = { _id: ObjectId(req.params.id) };
